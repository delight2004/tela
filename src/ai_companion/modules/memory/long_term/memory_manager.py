...
class MemoryAnalysis(BaseModel):
    ...
    is_important: bool = Field(..., description="Whether the message is important enough to be stored as memory.")
...
class MemoryManager:
    ...
    def __init__(self):
        ...
        self.llm = ChatGroq(
            ...
            temperature=0.1,
            max_retries=2,
        ).with_structured_output(schema=MemoryAnalysis)
    ...
    async def extract_and_store_memories(self, message: BaseMessage) -> None:
        ...
        if analysis.is_important and analysis.formatted_memory:
            ...
            self.logger.info(f"Storing new memory '{analysis.formatted_memory}' in vector store...")
    ...
    def format_memories_for_prompt(self, memories: List[str]) -> str:
        ...
        if not memories:
            return ""
        return "\n".join(f"- {memory}" for memory in memories) #md style bullet points
