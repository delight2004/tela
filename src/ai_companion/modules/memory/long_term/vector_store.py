...
@dataclass
class Memory:
    """Represents a memory entity in the vector class"""
    text: str
    metadata: str
...
# PERHAPS USE SETTINGS
    ...
    def store_memory(self, text: str, metadata: dict) -> None:
        """Store a new memory in the vector store or update if a similar one exists.
        ...
        point = PointStruct(
            ...
            vector=embedding.tolist(), # The embedding from step 1, converted to a list (necessary for Qdrant API compatibility).
            payload={
                "text": text,
                **metadata, # A dictionary containing "text" and all metadata key-value pairs (**metadata spreads all metadata fields into payload).
            },
        )
        self.client.upsert(
            collection_name=self.COLLECTION_NAME,
